...
export function canVerifyPayments(user: TokenPayload, cityId?: string): boolean {
...
export function canModeratePhoto(user: TokenPayload, cityId?: string): boolean {
  if (isSuperAdmin(user) || isRegionalAdmin(user)) return true;
  if (cityId) return isCityAdmin(user, cityId);
  return hasAnyRole(user, [RoleCode.SUPER_ADMIN, RoleCode.CITY_ADMIN]);
}

export function canVerifyMatchResult(user: TokenPayload, cityId: string): boolean {
  return isSuperAdmin(user) || isCityAdmin(user, cityId);
}
