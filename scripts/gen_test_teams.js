...
async function runTests() {
  ...
  try {
    // 1. Setup Test Users and Dynamic City / Sport
    const jampur = await prisma.city.upsert({
      where: { slug: 'jampur-test' },
      update: {},
      create: {
        name: 'Jampur Test',
        slug: 'jampur-test',
        code: 'JMP-T',
        region: { connectOrCreate: { where: { code: 'SP-TEST' }, create: { code: 'SP-TEST', name: 'South Punjab Test' } } },
      },
    });

    const cricket = await prisma.sport.upsert({
      where: { slug: 'cricket-test' },
      update: {},
      create: {
        name: 'Cricket Test',
        slug: 'cricket-test',
        ...
        category: { connectOrCreate: { where: { code: 'TEAM-T' }, create: { code: 'TEAM-T', name: 'Team Sports' } } },
      },
    });
    ...
    const captain = await prisma.user.create({
      data: {
        email: \`captain.test.\${Date.now()}@sports.pk\`,
    ...
    const athlete1 = await prisma.user.create({
      data: {
        email: \`athlete1.test.\${Date.now()}@sports.pk\`,
        passwordHash: captainPassword,
    ...
    const athlete2 = await prisma.user.create({
      data: {
        email: \`athlete2.test.\${Date.now()}@sports.pk\`,
        passwordHash: captainPassword,
    ...
    const superAdmin = await prisma.user.create({
      data: {
        email: \`admin.test.\${Date.now()}@sports.pk\`,
        passwordHash: captainPassword,
        fullName: 'Chief Sports Officer',
        homeCityId: jampur.id,
      },
    });

    const superAdminRole = await prisma.role.upsert({ where: { code: 'SUPER_ADMIN' }, update: {}, create: { code: 'SUPER_ADMIN', name: 'Super Admin' } });
    await prisma.userRole.create({ data: { userId: superAdmin.id, roleId: superAdminRole.id } });

    console.log('[Phase 1] Testing Team Creation & Yearly Registration Fee');
    ...
    assert(team.status === 'DRAFT', 'New team initializes with status DRAFT');
    assert(team.members.length === 1, 'Founding captain is automatically enrolled as active roster member');
    ...
    console.log('\n[Phase 2] Testing Payment Proof Submission & Status Advancement');
    ...
    console.log('\n[Phase 3] Testing Administrative Review Workflow (Active & Ranking Generation)');
    ...
    console.log('\n[Phase 4] Testing Player Invitation Lifecycle & Anti-Dual-Team Enforcement');
    ...
    const secondTeam = await prisma.team.create({
      data: {
        cityId: jampur.id,
        sportId: cricket.id,
        captainId: athlete2.id,
        name: \`DG Thunder \${Date.now()}\`,
        code: 'DGT',
        status: 'ACTIVE',
      },
    });
    ...
    console.log('\n[Phase 5] Testing Join Request Workflow');
    ...
    console.log('\n[Phase 6] Testing Historical Membership Retention (Never Hard Delete)');
    ...
    console.log('\n[Phase 7] Testing Team Profile Metrics, Winning Photos & Settings Updates');
    ...
    console.log(\`\n=== TEAM MANAGEMENT TESTS COMPLETE: \${passed} PASSED, \${failed} FAILED ===\`);
...
fs.writeFileSync('tests/test-teams-management.ts', testCode.trim() + '\n', 'utf8');
console.log('[OK] Created tests/test-teams-management.ts');
