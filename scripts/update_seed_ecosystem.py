...
async function main() {
  ...
  const dgKhanFalcons = await prisma.team.create({
    data: {
  ...
  const jampurUnitedFC = await prisma.team.create({
    data: {
      ...
      members: {
        create: [
          { playerId: captainAli.id, role: 'CAPTAIN', jerseyNumber: 9, status: 'ACTIVE' },
        ],
      },
    },
  });
...
"""
    ...
    print("[OK] Updated prisma/seed.ts")
