...
import { hasRole, RoleCode, canManageCity } from '../src/lib/rbac';
...
async function runTests() {
  ...
  const submittedPayment = await submitPaymentProof({
    ...
    transactionReference: `EP-TEST-${Date.now()}`,
  ...
  // Create an isolated test athlete for transfer tests
  const testAthlete = await prisma.user.create({
    data: {
      email: `test.athlete.${Date.now()}@sports.pk`,
      fullName: 'Test Athlete Bilal',
      passwordHash: hashed,
      homeCityId: jampur!.id,
    },
  });

  const originTeam = await prisma.team.create({
    data: {
      cityId: jampur!.id,
      sportId: cricket!.id,
      captainId: superAdmin!.id,
      name: `Origin Lions CC ${Date.now()}`,
      code: `O${Date.now().toString().slice(-4)}`,
      status: 'ACTIVE',
    },
  });

  const initialMembership = await prisma.teamMember.create({
    data: {
      teamId: originTeam.id,
      playerId: testAthlete.id,
      role: 'PLAYER',
      status: 'ACTIVE',
    },
  });
  assert(initialMembership.status === 'ACTIVE', 'Player has active membership in initial team');
  ...
  const transferOrder = await prisma.playerTransfer.create({
    data: {
      ...
      sportId: cricket!.id,
      cityId: jampur!.id,
      oldTeamId: originTeam.id,
      newTeamId: testTeam.id,
      status: 'PENDING_PAYMENT',
      fee: 100.0,
    },
  });
  assert(transferOrder.fee === 100.0, 'Player transfer fee configured as Rs. 100');

  // Process transfer approval atomically
  await prisma.teamMember.updateMany({
    where: { teamId: originTeam.id, playerId: testAthlete.id, status: 'ACTIVE' },
    data: { status: 'FORMER', leftAt: new Date() },
  });
  await prisma.teamMember.create({
    data: { teamId: testTeam.id, playerId: testAthlete.id, role: 'PLAYER', status: 'ACTIVE' },
  });
  ...
  const formerMem = await prisma.teamMember.findFirst({
    where: { teamId: originTeam.id, playerId: testAthlete.id, status: 'FORMER' },
  });
  const newMem = await prisma.teamMember.findFirst({
    where: { teamId: testTeam.id, playerId: testAthlete.id, status: 'ACTIVE' },
  });
  ...
  const matchRecord = await prisma.match.create({
    data: {
      ...
      participants: {
        create: [
          { teamId: originTeam.id, playerId: superAdmin!.id, isStarting: true },
          { teamId: testTeam.id, playerId: testAthlete.id, isStarting: true },
        ],
      },
      scorebook: {
        create: {
  ...
  await prisma.scoreEvent.createMany({
    data: [
      { scorebookId: matchRecord.scorebook!.id, matchId: matchRecord.id, eventType: 'RUNS', teamId: originTeam.id, playerId: superAdmin!.id, detailsJson: JSON.stringify({ runs: 6 }) },
      { scorebookId: matchRecord.scorebook!.id, matchId: matchRecord.id, eventType: 'RUNS', teamId: originTeam.id, playerId: superAdmin!.id, detailsJson: JSON.stringify({ runs: 4 }) },
      { scorebookId: matchRecord.scorebook!.id, matchId: matchRecord.id, eventType: 'WICKET', teamId: originTeam.id, playerId: superAdmin!.id, detailsJson: JSON.stringify({ bowlerId: testAthlete.id, isWicket: true }) },
    ],
  });

  const events = await prisma.scoreEvent.findMany({ where: { matchId: matchRecord.id } });
  const calculated = calculateScorebookState('CRICKET', originTeam.id, testTeam.id, events);
  ...
  const playerStats = await prisma.playerStatistic.findUnique({
    where: { playerId_sportId: { playerId: testAthlete.id, sportId: cricket!.id } },
  });
